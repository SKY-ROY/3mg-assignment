...
public class ObjectPooler : MonoBehaviour
{
    ...
    public GameObject GetPooledObject(string objectType, Vector3 reqPosition, Quaternion reqRotation, bool isActive = false, Transform parentTransform = null)
    {
        ...
        for (int i = 0; i < mPoolDictionary[objectType].Count; i++)
        {
            ...
            if (!mPoolDictionary[objectType][i].activeInHierarchy && mPoolDictionary[objectType][i].transform.parent == null)
            {
        ...
        if (canGrow)
        {
            foreach (Pooler pool in mPool)
            {
                if (pool.poolType == objectType)
                {
                    ...
                    {
                        obj = (GameObject)Instantiate(pool.objectPrefab);
                    }
                    else
                    {
                        obj = (GameObject)Instantiate(pool.objectPrefab, parentTransform);
                    }
